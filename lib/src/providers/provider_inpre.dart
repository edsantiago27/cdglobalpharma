...
class ProviderInpre extends ChangeNotifier {
  ...
  static Future cargaData(String folio) async {
    ...
    var response = await http.get('http://192.168.0.8:8182/proceso/$folio');
    if (response == null) {
      return false;
    }
    return response;
  ...
  static Future cargaUbicacion(String folio) async {
    var response =
        await http.get('http://192.168.0.8:8182/api/inprepeds/ubica/$folio');

    //var response = await http

    //  .get('http://192.168.0.111:8183/api/inprepeds/ubica/$folio');
    //var response = await http.get('http://192.168.0.8:8182/api/inprepeds/ubica/${folio}');
  ...
  static Future cargaLotes(String folio, String codigob) async {
    ...
    var response = await http.post('http://192.168.0.8:8182/picking/');
    return response;
  }
}
